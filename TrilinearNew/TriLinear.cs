namespace ThreeLinearInterpolation
{
    ...
    internal class InterpolateXS
    {
        ...
        public InterpolateXS(string inputText, string inputDataFileName) : this()
        {
        ...
        public void LinearInterpolation()
        {
            ...
            for (int i = 0; i < this.newFt.Length; i++)
            {
                ...
                int[] outputCoeffArray = this.FindPointsToInterpolate(fT, newFt, i);
                ...
                for (int j = 0; j < this.newMt.Length; j++)
                {
                    ...
                    outputCoeffArray = this.FindPointsToInterpolate(mT, newMt, j);
                    ...
                    for (int k = 0; k < this.newDm.Length; k++)
                    {
                        ...
                        outputCoeffArray = this.FindPointsToInterpolate(Dm, newDm, k);
        ...
        private void SecondInterpolationVariant(int ix0, int ix1, int iy0, int iy1, int iz0, int iz1, int i, int j, int k)
        {
            ...
            fT_norm = this.NormalizeDistance(fT, newFt, ix0, ix1, i);
            mT_norm = this.NormalizeDistance(mT, newMt, iy0, iy1, j);
            dM_norm = this.NormalizeDistance(Dm, newDm, iz0, iz1, k);
        ...
        private void ConvertTextToNumbers()
        {
            ...
            {
                this.InputText = this.InputText.Replace(c.ToString(), "");
            }
